...
Game.prototype.update = function(){
	for(var i in this.rockets){
		if(this.rockets[i].state == rocketStates.flying){
			...
			var res = compute(inputs, this.rockets[i].network);
...
Game.prototype.display = function(){
	...
	this.ctx.fillText("Generation : "+this.generationCount, 10, 25);
	this.ctx.fillText("Landed : "+this.landedCount+" / "+(generation == undefined ? 0 : generation.length), 10, 50);
...
function post(params, callback){
    ...
    xmlHttp.onreadystatechange = function()
    {
        if(xmlHttp.readyState == 4)
        {
            ...
            {
                callback(xmlHttp.responseText, undefined);
            }else{
...
function getNextGeneration(callback){
	post({
		"url": "Neuroevolution4j/",
		"body": generation
	}, function(data, err){
		if(err){
			console.error(err)
		}else{
			generation = JSON.parse(data);
			callback();
		}
	});
}

function compute(inputs, network) {
	...
	for (var i = 1; i < layers.length; i++) {
		for (var j in layers[i].neurons) {
			...
			layers[i].neurons[j].value = activation(sum);
		}
		prevLayer = layers[i];
	}
	...
	//Output Layer
	var output = [];
	for (var i in outputLayer.neurons) {
		output.push(outputLayer.neurons[i].value);
	}
	
	return output;
}

const activation = (x) => (1 / (1 + Math.exp((-x) / 1)));